...
def get_pydantic_model(
    ...
    *,
    feature_id: str = "hermes_llm",
):
    """Return a model from the global feature binding.

    Legacy provider and model arguments are accepted only for source
    compatibility and are intentionally ignored. Runtime model selection is
    never allowed to come from a project, environment preset, or request.
    """
    del provider_override, model_name_override
    ...
    return _newapi_text_openai_model(
        ...
        timeout_seconds=_env_float("NEWAPI_TEXT_TIMEOUT_SECONDS", 120.0),
        profile=_get_newapi_text_model_profile(resolved.model_id),
    )
...
def get_newapi_text_pydantic_model(
    ...
    *,
    ...
    """Compatibility wrapper using the global Hermes feature binding."""

    del model_env, default_model, model_name_override, timeout_seconds_override
    return get_pydantic_model(feature_id="hermes_llm")
...
def get_superpower_pydantic_model(
    *,
    ...
    """Return the globally bound vision model used by prompt builders."""
    del feature_provider_env, feature_model_env
    return get_pydantic_model(feature_id="vision_analysis_llm")
...
def get_model_info() -> dict:
    """Return the globally bound default text model."""

    from novelvideo.model_catalog import model_runtime_resolver

    resolved = model_runtime_resolver.resolve("hermes_llm")
    return {
        "provider": "newapi",
...
def get_newapi_runtime_credentials(
    *,
    ...
    process environment. Per-call credential overrides are retained in the
    signature for compatibility but are intentionally ignored.
    """

    del api_key_override, base_url_override, env_api_key, env_base_url
    ...
    if environment_backed:
        api_key = api_key or NEWAPI_API_KEY
        base_url = base_url or str(NEWAPI_BASE_URL or "").strip().rstrip("/")
...
def _global_image_generation_config(feature_id: str, *, image_size: str = "1K") -> dict:
    from novelvideo.model_catalog import model_runtime_resolver
...
def get_grid_generation_config(
    ...
    """Return the global render-image binding; legacy overrides are ignored."""
    del selection_override, provider_override, model_override
    return _global_image_generation_config(
        "render_image", image_size=image_size_override or "1K"
    )


def get_character_reference_generation_config(
    selection_override: str | None = None,
    model_override: str | None = None,
) -> dict:
    """Return the global character-reference-image binding; overrides are ignored."""
    del selection_override, model_override
    return _global_image_generation_config("character_reference_image")
...
def get_sketch_generation_config(
    ...
    """Return the global sketch-image binding; project overrides are ignored."""
    del selection_override, model_override
    config = _global_image_generation_config("sketch_image")
...
def get_render_generation_config(
    ...
    """Return the global render-image binding; project overrides are ignored."""
    del selection_override, model_override
    return _global_image_generation_config("render_image")
