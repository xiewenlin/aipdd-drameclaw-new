...
def _project_has_cognee_data(state_dir: str | Path) -> bool:
    root = Path(state_dir) / "cognee_system"
    return root.exists() and any(path.is_file() for path in root.rglob("*"))


def _stored_embedding_signature(config: dict) -> dict[str, object] | None:
    from novelvideo.embedding_models import PROJECT_EMBEDDING_SIGNATURE_KEY

    value = config.get(PROJECT_EMBEDDING_SIGNATURE_KEY)
    return dict(value) if isinstance(value, dict) else None


def save_cognee_embedding_signature_in_state_dir(
    state_dir: str | Path,
    ...
    effective_spec = spec or embedding_model_spec()
    signature = embedding_signature(effective_spec)

    def _apply(config: dict) -> None:
        config[PROJECT_EMBEDDING_DIMENSION_KEY] = effective_spec.dimensions
        config[PROJECT_EMBEDDING_SIGNATURE_KEY] = signature
...
def ensure_cognee_embedding_binding_in_state_dir(
    ...
    """Resolve project dimensions and safely adopt a signature for an empty knowledge base."""
    ...
    has_cognee_data = _project_has_cognee_data(state_dir)
    if not has_cognee_data and _stored_embedding_signature(config) is not None:
        spec = embedding_model_spec()
    else:
        spec = embedding_model_spec(dimensions=dimensions)
    if not has_cognee_data:
        save_cognee_embedding_signature_in_state_dir(state_dir, spec)
    return spec


def require_cognee_embedding_compatibility_in_state_dir(
    state_dir: str | Path,
) -> EmbeddingModelSpec:
    from novelvideo.embedding_models import (
        ...
        embedding_model_spec,
        embedding_signature,
        embedding_signature_summary,
    )

    spec = embedding_model_spec()
    current = embedding_signature(spec)
    config = load_project_config_file_from_state_dir(state_dir)
    stored = _stored_embedding_signature(config)
    if not _project_has_cognee_data(state_dir):
        save_cognee_embedding_signature_in_state_dir(state_dir, spec)
        return spec
    if stored is None:
        raise KnowledgeEmbeddingRebuildRequired(
            "knowledge_embedding_rebuild_required: existing knowledge base has no reliable "
            f"Embedding signature; rebuild with {embedding_signature_summary(current)}"
        )
    if stored.get("fingerprint") != current.get("fingerprint"):
        raise KnowledgeEmbeddingRebuildRequired(
            "knowledge_embedding_rebuild_required: existing "
            f"{embedding_signature_summary(stored)}; current "
            f"{embedding_signature_summary(current)}"
        )
    return spec

def ensure_cognee_embedding_model_in_state_dir(state_dir: str | Path) -> str:
    """Compatibility wrapper returning the global bound model name."""
