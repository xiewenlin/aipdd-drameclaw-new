...
async def _call_vision_model(*, image_path, prompt, model, max_tokens):
    ...
    _, result = await call_freezone_vision_model(
        prompt=prompt,
        images=[VisionInput(data=image_path.read_bytes(), media_type="image/jpeg")],
    )
    return result
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--api-key", default="", help="Deprecated. Key is now managed via NewAPI connection.")
