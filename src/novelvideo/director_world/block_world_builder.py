...
def resolve_model_config(args: argparse.Namespace) -> tuple[str, str, str | None]:
    """Resolve BlockWorld exclusively from the global scene-builder binding."""

    del args
    from novelvideo.model_catalog import model_runtime_resolver

    resolved = model_runtime_resolver.resolve("scene_builder_llm")
    return resolved.model_id, resolved.api_key, resolved.base_url
...
def main() -> None:
    ...
    if args.from_code:
        ...
        if not api_key:
            raise SystemExit(
                "Model API key is missing; configure scene_builder_llm binding in NewAPI settings, "
                "or use --prompt-only / --from-code for offline testing"
            )
