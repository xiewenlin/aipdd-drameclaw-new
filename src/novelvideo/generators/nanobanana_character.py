...
from novelvideo.config import (
    get_character_reference_generation_config,
    get_style_preset,
    IMAGE_DEFAULT_STYLE,
)
...
class NanoBananaCharacterGenerator:
    ...
    def __init__(
        ...
        """
        config = config or get_character_reference_generation_config(
        ...
        if self.provider != "newapi":
            raise ValueError(
                f"Unsupported character generation provider: {self.provider}. "
...
"""
        ...
        prompt = f"""Generate a face-only character identity reference portrait of {character_tag} ({character_name}).

FACIAL FEATURES TO CAPTURE:
{character_prompt}

VISUAL STYLE:
Realistic skin and natural facial detail, soft lighting, restrained cinematic color.

REQUIREMENTS:
- Perfectly front-facing, looking directly at camera, neutral expression, mouth closed
- Head-and-shoulders close-up; face fills 60-70% of frame
- Plain matte gray background and simple dark top
- Preserve face shape, eyes, nose, mouth, hairstyle, skin tone, and age

MUST AVOID:
Side view, full body, elaborate costume, accessories, props, scenery, multiple people, text, watermark, anime, illustration, plastic skin, beauty filters, deformed face, or AI artifacts.
"""
        return prompt.strip()
...
"""
    ...
    async def _generate_single_image(
        ...
        try:
            ...
            print(f"[NanoBanana Character] ?? NewAPI ({self.model}) ????...")
            ...
            if not image_bytes and error_detail:
                print(f"[NanoBanana Character] NewAPI ????: {error_detail}")
