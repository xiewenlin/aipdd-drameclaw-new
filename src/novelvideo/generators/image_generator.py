...
class VolcengineImageGenerator:
    ...
    def __init__(
        ...
        if api_key is not None or endpoint is not None:
            raise ValueError(
                "Image API overrides are not supported; configure render_image in Settings."
            )
        resolved = model_runtime_resolver.resolve("render_image")
        self.api_key = resolved.api_key
        self.endpoint = resolved.base_url
        self.seedream_model = resolved.model_id
        self.seededit_model = resolved.model_id
