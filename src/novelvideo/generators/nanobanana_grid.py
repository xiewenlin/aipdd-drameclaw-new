...
_VOLCENGINE_SEEDREAM_MIN_PIXELS = 3_686_400
...
NEWAPI_IMAGE_POLL_INTERVAL_SECONDS = 2.0
NEWAPI_IMAGE_MAX_POLLS = 900

_NEWAPI_IMAGE_SUCCESS_STATUSES = {"completed", "succeeded", "success", "done"}
_NEWAPI_IMAGE_FAILURE_STATUSES = {
    "failed",
    "error",
    "canceled",
    "cancelled",
...
_NEWAPI_IMAGE_PENDING_STATUSES = {
    "queued",
    "pending",
    "processing",
    "running",
...
def _newapi_image_status(payload: object) -> str:
    ...
    for key in ("data", "result"):
        status = _newapi_image_status(payload.get(key))
        if status:
            return status
    return ""


def _newapi_image_error(payload: object) -> str:
    if not isinstance(payload, dict):
        return "DramaClawAPI image task failed"
    error = payload.get("error") or payload.get("fail_reason")
    ...
    for key in ("data", "result"):
        ...
        if isinstance(nested, dict):
            nested_error = _newapi_image_error(nested)
            if nested_error != "DramaClawAPI image task failed":
                return nested_error
    message = payload.get("message")
    return str(message) if message else "DramaClawAPI image task failed"


def _newapi_image_output(payload: object) -> tuple[str, str]:
    ...
    if isinstance(payload, list):
        for item in payload:
            image_b64, image_url = _newapi_image_output(item)
    ...
    image_b64 = str(payload.get("b64_json") or payload.get("image_base64") or "").strip()
    image_url = str(payload.get("url") or payload.get("image_url") or "").strip()
    if image_b64 or image_url:
        return image_b64, image_url

    for key in ("data", "result", "output", "images", "image_urls"):
        nested = payload.get(key)
        if nested is payload:
            continue
        image_b64, image_url = _newapi_image_output(nested)
        if image_b64 or image_url:
            return image_b64, image_url
    return "", ""
...
def resolve_openai_image_size(
    ...
    image_size: str = "1K",
    *,
    min_pixels: int = _OPENAI_MIN_PIXELS,
) -> str:
    ...
    effective_min_pixels = max(_OPENAI_MIN_PIXELS, int(min_pixels or _OPENAI_MIN_PIXELS))
...
def resolve_newapi_image_size(
    ...
    model_name = str(model or "").strip().lower()
    min_pixels = (
        _VOLCENGINE_SEEDREAM_MIN_PIXELS
        if "seedream" in model_name
        else _OPENAI_MIN_PIXELS
    )
    return resolve_openai_image_size(
        aspect_ratio,
        image_size,
...
async def _call_newapi_image_api(
    *,
    ...
    size = resolve_newapi_image_size(aspect_ratio, image_size, model)
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            image_b64, image_url = _newapi_image_output(result)

            if not image_b64 and not image_url and not task_id:
                fallback_id = str(result.get("id") or "").strip()
                status = _newapi_image_status(result)
                if fallback_id and status:
                    task_id = fallback_id
            ...
            if not image_b64 and not image_url and task_id:
                logger.info(
                    "DramaClawAPI image task accepted: task_id=%s status=%s",
                    task_id,
                    _newapi_image_status(result) or "queued",
                )
                for poll_count in range(NEWAPI_IMAGE_MAX_POLLS):
                    status = _newapi_image_status(result)
                    if status in _NEWAPI_IMAGE_FAILURE_STATUSES:
                        error = _newapi_image_error(result)
                        await _refund(
                            reservation_id,
                            "newapi_image_api",
                            error,
                        ...
                        return None, "", f"DramaClawAPI image task failed: {error}"
                    if status in _NEWAPI_IMAGE_SUCCESS_STATUSES:
                        await _refund(
                            reservation_id,
                            "newapi_image_api",
                        ...
                        return None, "", (
                            "DramaClawAPI image task completed without image data: "
                            f"task_id={task_id}"
                        )

                    if poll_count > 0 or status not in _NEWAPI_IMAGE_PENDING_STATUSES:
                        logger.info(
                            "DramaClawAPI image task polling: task_id=%s status=%s poll=%d/%d",
                            task_id,
                            status or "queued",
                            poll_count + 1,
                            NEWAPI_IMAGE_MAX_POLLS,
                        )
                    if poll_count > 0 or status in _NEWAPI_IMAGE_PENDING_STATUSES:
                        await asyncio.sleep(NEWAPI_IMAGE_POLL_INTERVAL_SECONDS)

                    task_response = await client.get(
                        f"{endpoint}/images/generations/{task_id}",
                        headers=headers,
                    )
                    task_response.raise_for_status()
                    task_headers = getattr(task_response, "headers", {}) or {}
                    provider_request_id = (
                        _newapi_request_id_from_headers(task_headers) or provider_request_id
                    )
                    ...
                    return None, "", (
                        "Timeout waiting for DramaClawAPI image task: "
                        f"task_id={task_id}"
                    )
            ...
            return None, "", "DramaClawAPI Images response missing b64_json/url"
...
class NanoBananaGridGenerator:
    ...
    def __init__(self, api_key: Optional[str] = None, config: Optional[dict] = None):
        ...
        self.provider = config.get("provider", "newapi")
        ...
        if self.provider != "newapi":
            raise ValueError(
                f"Unsupported grid provider: {self.provider}. "
                "All image generation must use the NewAPI gateway configured in settings."
            )
        if not self.api_key:
            raise ValueError("NewAPI API key is not configured. Please configure it in settings.")
