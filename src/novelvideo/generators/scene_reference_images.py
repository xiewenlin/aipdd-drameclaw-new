...
def resolve_scene_reference_image_model(
    ...
    *,
    ...
    """Return the globally bound model used by scene reference generation."""
    del provider, model
    from novelvideo.model_catalog import model_runtime_resolver

    feature_id = "scene_reverse_image" if kind == "reverse_master" else "scene_master_image"
    return model_runtime_resolver.resolve(feature_id).model_id
