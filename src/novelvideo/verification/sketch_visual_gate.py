...
def _resolve_gate_backend() -> tuple[str, str, str]:
    ...
    resolved = model_runtime_resolver.resolve("vision_analysis_llm")
    return "newapi", resolved.api_key, resolved.model_id
...
async def _ask_vlm_once(
    *,
    ...
    del provider, api_key, max_tokens
    from novelvideo.freezone.vision_gateway import VisionInput, call_freezone_vision_model
    _, result = await call_freezone_vision_model(
        prompt=prompt,
        images=[VisionInput(data=image_bytes, media_type="image/png")],
    )
    return result
...
async def gate_candidate_cells(
    *,
    ...
    if not provider or not api_key:
        raise RuntimeError(
            "Gate requires a VLM key. Configure the vision_analysis_llm binding "
            "in NewAPI settings."
        )
