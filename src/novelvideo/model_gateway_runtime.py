...
def _cognee_runtime_status() -> str:
    from novelvideo.model_runtime_hot_reload import get_model_runtime_status

    return str(get_model_runtime_status()["runtimeState"])
...
def refresh_model_gateway_runtime() -> dict[str, Any]:
    ...
    Cognee converges through process-local generation leases; Hermes performs
    its own worker fingerprint rotation.
    """
