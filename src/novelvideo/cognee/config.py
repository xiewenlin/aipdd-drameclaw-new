...
def cognee_gateway_restart_required() -> bool:
    """Return whether the temporary rollback mode requires a process restart."""
    from novelvideo.model_runtime_hot_reload import model_runtime_hot_reload_enabled

    return not model_runtime_hot_reload_enabled()


def _resolve_llm_api_key(llm_provider: str, llm_model: str) -> str:
    # Always use the configured NewAPI gateway
...
def _clear_cognee_embedding_config_cache(*, strict: bool = False) -> None:
    try:
        mod = sys.modules.get(
...
def _clear_cognee_llm_config_cache(*, strict: bool = False) -> None:
...
def _clear_litellm_client_cache(*, strict: bool = False) -> None:
    """Reset LiteLLM's process-local client cache when credentials change."""
    try:
        litellm_module = sys.modules.get("litellm")
        if litellm_module is None:
            return
        client_cache = getattr(litellm_module, "in_memory_llm_clients_cache", None)
        flush_cache = getattr(client_cache, "flush_cache", None)
...
def _project_embedding_request_kwargs(kwargs: dict) -> dict:
    """Apply the global embedding binding to LiteLLM kwargs."""

    spec = require_current_embedding_model_spec()
    api_key, api_base = embedding_gateway_credentials(spec)
    routed = dict(kwargs)
...
def _apply_embedding_env(llm_provider: str, api_key: str) -> tuple[str, str, str, str]:
    ...
    embedding_api_key = _effective_newapi_gateway()[0] or api_key
...
try:
    ...
    os.environ["LLM_PROVIDER"] = cognee_llm_provider
    os.environ["LLM_API_KEY"] = api_key
...
def _apply_runtime_snapshot(snapshot) -> None:
    """Atomically apply one immutable model runtime snapshot to Cognee."""
    ...
    previous_config = {
        key: getattr(cognee.config, key)
        for key in config_attrs
        if hasattr(cognee.config, key)
    }

    try:
        llm_provider = _resolve_llm_provider()
        llm_model = _normalize_llm_model(llm_provider, snapshot.llm_model)
        cognee_provider = _to_cognee_provider(llm_provider)
        embedding_provider = cognee_provider
        embedding_model = _normalize_embedding_model(llm_provider, snapshot.embedding_model)
        embedding_dimensions = int(snapshot.embedding_dimensions)
        embedding_endpoint = snapshot.embedding_base_url or snapshot.base_url
        embedding_api_key = snapshot.embedding_api_key or snapshot.api_key

        os.environ["COGNEE_LLM_MODEL"] = snapshot.llm_model
        os.environ["COGNEE_LLM_ENDPOINT"] = snapshot.base_url
        os.environ["COGNEE_EMBEDDING_MODEL"] = snapshot.embedding_model
        os.environ["COGNEE_EMBEDDING_ENDPOINT"] = embedding_endpoint
        _apply_llm_env(llm_provider, llm_model, snapshot.api_key)
        _clear_cognee_llm_config_cache(strict=True)
        os.environ["EMBEDDING_PROVIDER"] = embedding_provider
        os.environ["EMBEDDING_MODEL"] = embedding_model
        os.environ["EMBEDDING_DIMENSIONS"] = str(embedding_dimensions)
        os.environ["EMBEDDING_API_KEY"] = embedding_api_key
        os.environ["EMBEDDING_ENDPOINT"] = embedding_endpoint
        _clear_cognee_embedding_config_cache(strict=True)
        _clear_litellm_client_cache(strict=True)
        _apply_cognee_runtime_defaults()

        cognee.config.llm_provider = cognee_provider
        cognee.config.llm_model = llm_model
        cognee.config.llm_api_key = snapshot.api_key
        cognee.config.embedding_provider = embedding_provider
        cognee.config.embedding_model = embedding_model
        cognee.config.embedding_dimensions = embedding_dimensions
        cognee.config.embedding_api_key = embedding_api_key
        setters = {
            "set_llm_provider": cognee_provider,
            "set_llm_model": llm_model,
            "set_llm_api_key": snapshot.api_key,
        ...
        for name, value in setters.items():
            setter = getattr(cognee.config, name, None)
            if callable(setter):
                setter(value)
        ...
        for key, value in previous_config.items():
            setattr(cognee.config, key, value)
        _clear_cognee_llm_config_cache()
        _clear_cognee_embedding_config_cache()
...
def init_cognee() -> None:
    """Ensure the latest persisted Cognee model configuration is active."""

    from novelvideo.model_runtime_hot_reload import (
        model_runtime_coordinator,
        model_runtime_hot_reload_enabled,
    )

    if model_runtime_hot_reload_enabled():
        model_runtime_coordinator.ensure_ready_sync()
    else:
        model_runtime_coordinator.ensure_ready_without_reload_sync()


from novelvideo.model_runtime_hot_reload import model_runtime_coordinator

model_runtime_coordinator.register_applier(_apply_runtime_snapshot)
