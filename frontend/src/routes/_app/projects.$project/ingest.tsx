...
export function IngestPageContent({ project }: { project: string }) {
  ...
  const rebuildKnowledgeGraphMutation = useRebuildKnowledgeGraph(project);
  ...
  const [graphRebuildConfirmOpen, setGraphRebuildConfirmOpen] = useState(false);
  ...
  const handleRebuildKnowledgeGraph = useCallback(async () => {
    try {
      setGraphRebuildConfirmOpen(false);
      setIngestLogs([]);
      setIngestError(null);
      await rebuildKnowledgeGraphMutation.mutateAsync();
      setIngestSubmitted(true);
      setIngestStarted(true);
      setIngestFileStatus("importing");
    } catch (error) {
  ...
  }, [rebuildKnowledgeGraphMutation, t]);
        ...
        <div className="mx-auto w-full max-w-[1080px]">
          {!shouldShowPreview ? (
            ...
            <div className="min-w-0 space-y-6">
              ...
              <AlertDialog
                open={graphRebuildConfirmOpen}
                onOpenChange={setGraphRebuildConfirmOpen}
              >
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>
                      {t("ingest.knowledgeGraph.rebuildTitle")}
                    </AlertDialogTitle>
                    <AlertDialogDescription>
                      {t("ingest.knowledgeGraph.rebuildDescription")}
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>{t("common.cancel")}</AlertDialogCancel>
                    ...
                    >
                      {t("ingest.knowledgeGraph.rebuildConfirm")}
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
              </AlertDialog>
                    ...
                    <div className="flex min-h-28 items-center justify-between gap-4 rounded-xl border border-amber-300/15 bg-amber-500/[0.04] p-4">
                      ...
                      <div className="flex shrink-0 gap-2">
                        ...
                        <Button
                          ...
                          variant="default"
                          size="sm"
                          disabled={rebuildKnowledgeGraphMutation.isPending}
                          onClick={() => setGraphRebuildConfirmOpen(true)}
                        >
                          {rebuildKnowledgeGraphMutation.isPending ? (
                            <Loader2 className="size-3.5 animate-spin" />
                          ) : (
                            <RefreshCw className="size-3.5" />
                          )}
                          {t("ingest.knowledgeGraph.rebuildAction")}
                        </Button>
                      </div>
                    </div>
                  )}
