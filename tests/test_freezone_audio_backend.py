...
from novelvideo import config, model_catalog
...
async def test_freezone_audio_eleven_music_uses_newapi_music_metadata(
    ...
    monkeypatch.setattr(
        ...
        "resolve",
        lambda feature_id: SimpleNamespace(model_id="global-audio-model"),
    )
    ...
    assert result.model == "global-audio-model"
    assert result.duration_ms == 30_000
    assert result.voice_source == "global-audio-model"
    assert calls == [
        {
            "output_path": freezone_audio_eleven_music_output_path(tmp_path, "music-1"),
            "model": "global-audio-model",
