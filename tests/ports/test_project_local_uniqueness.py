...
@pytest.mark.asyncio
async def test_non_local_owner_uses_same_identity_for_lookup_and_listing(local_registry):
    owner_id = "user-123"
    ...
    resolved = await local_registry.get_project_by_owner_name(owner_id, "agent")
    accessible = await local_registry.list_accessible_projects([("user", owner_id)])

    assert resolved is not None
    assert resolved.id == project.id
    assert [item.id for item in accessible] == [project.id]
