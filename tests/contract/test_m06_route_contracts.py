...
@pytest.mark.parametrize("backend", ["inline", "celery"])
def test_m06_ingest_rebuild_queues_canonical_novel(m06_client_factory, backend: str):
    client, task_backend, _task_manager, project_dir, _assets, _store = m06_client_factory(
        backend
    )
    novel_path = project_dir / "novel.txt"
    novel_path.write_text("Chapter 1\nA valid canonical novel.", encoding="utf-8")

    response = client.post(f"/api/v1/projects/{_PROJECT}/ingest/rebuild")
    payload = _assert_ok(response)

    assert payload["task_type"] == "ingest_fast"
    assert payload["backend"] == backend
    assert len(task_backend.calls) == 1
    call = task_backend.calls[0]
    assert call["task_type"] == "ingest_fast"
    assert call["payload"]["novel_path"] == str(novel_path)
    assert call["payload"]["config"] == {"rebuild": True}


def test_m06_ingest_rebuild_requires_canonical_novel(m06_client_factory):
    client, task_backend, _task_manager, _project_dir, _assets, _store = m06_client_factory(
        "inline"
    )
    ...
    payload = response.json()
    assert payload["ok"] is False
    assert payload["code"] == "NOVEL_IMPORT_REQUIRED"
